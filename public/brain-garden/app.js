...
const state={lang:'en',selected:'frontal',mode:false,arrangement:'assembled',ready:false,error:null,focused:false,pendingFocus:null};
...
function select(id,extract=true){state.selected=id;if(extract&&!state.ready)state.pendingFocus=id;if(extract&&state.ready){state.focused=true;state.arrangement='focused';state.mode=false;const right=new THREE.Vector3().setFromMatrixColumn(camera.matrix,0);parts.forEach(p=>p.userData.target.copy(p.userData.region===id?right.clone().multiplyScalar(3.4):new THREE.Vector3()));zoom(Math.max(1,12*Math.max(1,430/viewport.clientWidth)/camera.position.distanceTo(controls.target)));}if(state.focused)showFocus();labels();highlight()}
function showFocus(){
 ...
 focusModel=new THREE.Group();meshes.filter(m=>m.userData.region===state.selected).forEach(m=>{const copy=new THREE.Mesh(m.geometry,m.material.clone());copy.material.transparent=false;copy.material.opacity=1;copy.material.depthWrite=true;copy.material.emissiveIntensity=0;focusModel.add(copy)});
...
function reassemble(reset=false){if(!state.ready)return;clearFocus();parts.forEach(p=>p.userData.target.set(0,0,0));state.arrangement='assembled';setMode(false);if(reset){controls.target.set(0,0,0);camera.position.copy(home).multiplyScalar(Math.max(1,390/viewport.clientWidth));select('frontal',false);controls.update()}labels()}
...
async function start(){
 ...
 let lastWidth=0;const resize=()=>{const w=viewport.clientWidth,h=viewport.clientHeight;if(w!==lastWidth&&w<390){const d=camera.position.distanceTo(controls.target);zoom(Math.max(1,home.length()*390/w/d))}lastWidth=w;renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix()};new ResizeObserver(resize).observe(viewport);resize();
 ...
 const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;renderer.setAnimationLoop(()=>{parts.forEach(p=>{if(!drag||drag.part!==p)p.position.lerp(p.userData.target,reduced?1:.14)});controls.update();renderer.render(scene,camera);if(focusRenderer&&state.focused){focusControls.update();focusRenderer.render(focusScene,focusCamera)}});
 try{
 ...
 // Group named surfaces by region and hemisphere. Preserve their anatomical geometry.
 const side=/^left-/.test(source.name)?'left':/^right-/.test(source.name)?'right':center.x<-.1?'left':center.x>.1?'right':'mid';const groupKey=id+'-'+side;
 let p=groupMap.get(groupKey);if(!p){p=new THREE.Group();p.name=groupKey;p.userData={region:id,target:new THREE.Vector3()};groupMap.set(groupKey,p);scene.add(p);parts.push(p)}
 ...
 parts.forEach(p=>{const c=new THREE.Box3().setFromObject(p).getCenter(new THREE.Vector3());const dir=c.clone().normalize();if(p.name.endsWith('left'))dir.x=-Math.max(.5,Math.abs(dir.x));if(p.name.endsWith('right'))dir.x=Math.max(.5,Math.abs(dir.x));p.userData.explode=dir.multiplyScalar(['frontal','parietal','temporal','occipital','cerebellum'].includes(p.userData.region)?1.2:.7)});
 state.ready=true;$('#loading').hidden=true;['separate','reassemble','drag-mode'].forEach(id=>$('#'+id).disabled=false);document.querySelectorAll('[data-move]').forEach(b=>b.disabled=false);highlight();if(state.pendingFocus){const pending=state.pendingFocus;state.pendingFocus=null;select(pending)};
