...
import {regions,ui} from './content.js';
const $=s=>document.querySelector(s);
const state={lang:'en',selected:'frontal',mode:false,arrangement:'assembled',ready:false,error:null};
...
regions.forEach(r=>{const b=document.createElement('button');b.dataset.region=r.id;b.style.setProperty('--color',r.color);b.innerHTML='<span class="swatch" aria-hidden="true"></span><span class="region-text"></span>';b.onclick=()=>{select(r.id);$('#notes').scrollIntoView({block:'nearest',behavior:'instant'})};$('#region-list').append(b)});
function select(id){state.selected=id;labels();highlight()}
function highlight(){meshes.forEach(m=>{const yes=m.userData.region===state.selected;const h=m.userData.region===hover;m.material.emissive.set(m.material.color);m.material.emissiveIntensity=yes?.18:h?.10:0;m.material.roughness=yes?.63:.78})}
...
function reassemble(reset=false){if(!state.ready)return;parts.forEach(p=>p.userData.target.set(0,0,0));state.arrangement='assembled';setMode(false);if(reset){controls.target.set(0,0,0);camera.position.copy(home).multiplyScalar(Math.max(1,390/viewport.clientWidth));select('frontal');controls.update()}labels()}
$('#reassemble').onclick=()=>reassemble();$('#reset').onclick=()=>reassemble(true);
$('#separate').onclick=()=>{parts.forEach(p=>p.userData.target.copy(p.userData.explode));state.arrangement='separated';setMode(true);const distance=camera.position.distanceTo(controls.target);zoom(Math.max(1,10*Math.max(1,390/viewport.clientWidth)/distance));labels()};
...
async function start(){
 ...
 controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.dampingFactor=.08;controls.enablePan=false;controls.minDistance=4.5;controls.maxDistance=15;controls.rotateSpeed=.6;
 ...
 const finish=e=>{if(drag&&drag.id===e.pointerId){e.stopImmediatePropagation();if(renderer.domElement.hasPointerCapture(e.pointerId))renderer.domElement.releasePointerCapture(e.pointerId);drag=null;viewport.style.cursor='grab';labels()}else if(down&&e.type==='pointerup'&&Math.hypot(e.clientX-down.x,e.clientY-down.y)<6){const h=hit(e);if(h)select(h.object.userData.region)}down=null};
 ...
 const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;renderer.setAnimationLoop(()=>{parts.forEach(p=>{if(!drag||drag.part!==p)p.position.lerp(p.userData.target,reduced?1:.14)});controls.update();renderer.render(scene,camera)});
 try{
 ...
 state.ready=true;$('#loading').hidden=true;['separate','reassemble','drag-mode'].forEach(id=>$('#'+id).disabled=false);document.querySelectorAll('[data-move]').forEach(b=>b.disabled=false);highlight();
