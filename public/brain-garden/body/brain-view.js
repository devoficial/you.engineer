...
export class PathwayBrain{
 constructor(host,onSelect=()=>{}){
  this.host=host;this.onSelect=onSelect;this.meshes=[];this.locators=[];this.active=false;this.innerOverride=null;this.phase=0;this.viewChanged=false;this.routeLabels=[];this.motionTime=0;this.running=!matchMedia('(prefers-reduced-motion: reduce)').matches;this.lastFrame=0;
  this.overlay=document.createElement('div');this.overlay.className='pathway-callouts';host.append(this.overlay);
  ...
  this.controls.addEventListener?.('start',()=>{this.cameraGoal=null});this.controls.addEventListener?.('change',()=>{if(!this.cameraGoal)this.checkView()});
  ...
  host.addEventListener('keydown',e=>{if(e.target!==host||!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','=','-'].includes(e.key))return;e.preventDefault();this.cameraGoal=null;const s=new THREE.Spherical().setFromVector3(this.camera.position);if(['+','=','-'].includes(e.key))s.radius=THREE.MathUtils.clamp(s.radius*(e.key==='-'?1.1:.9),4,13);else{s.theta+=e.key==='ArrowLeft'?.15:e.key==='ArrowRight'?-.15:0;s.phi=THREE.MathUtils.clamp(s.phi+(e.key==='ArrowUp'?-.15:e.key==='ArrowDown'?.15:0),.1,Math.PI-.1)}this.camera.position.setFromSpherical(s);this.controls.update();this.checkView()});
 ...
 select(steps,index){this.steps=steps;this.index=index;this.step=steps[index];if(this.failed)return;const visited=steps.slice(0,index);this.inner=this.innerOverride??Boolean(this.step.inner);this.highlighted=[];
  ...
  if(this.signalTrain){this.scene.remove(this.signalTrain);this.signalTrain.geometry.dispose();this.signalTrain.material.dispose();this.signalTrain=null}
  if(this.bundle){this.scene.remove(this.bundle);this.bundle.traverse(o=>{o.geometry?.dispose();o.material?.dispose()});this.bundle=null}
  this.overlay.replaceChildren();this.routeLabels=[];
  ...
  const to=point(index);const targetKey=this.step.id+JSON.stringify(this.step.meshes||this.step.regions||this.step.loci||[]);if(to&&targetKey!==this.targetKey&&this.innerOverride===null){const side=to.x<-.08?-1:1;this.cameraGoal=new THREE.Vector3(side*5.6,2.5,to.z<-.25?-4.6:to.z>.25?4.6:2.4);this.homePosition=this.cameraGoal.clone();this.viewChanged=false;this.host.dispatchEvent(new CustomEvent('brain-view-changed'));this.controls.target.set(0,0,0);if(matchMedia('(prefers-reduced-motion: reduce)').matches){this.camera.position.copy(this.cameraGoal);this.cameraGoal=null}this.controls.update()}this.targetKey=to?targetKey:null;let from=null,fromIndex=null;for(let i=index-1;i>=0&&!from;i--){from=point(i);if(from)fromIndex=i}
  if(to&&from&&to.distanceTo(from)>.06){const mid=from.clone().lerp(to,.5).add(new THREE.Vector3(0,.45,.2));this.curve=new THREE.CatmullRomCurve3([from,mid,to]);this.connection=new THREE.Mesh(new THREE.TubeGeometry(this.curve,40,.018,8,false),new THREE.MeshBasicMaterial({color:colors[this.step.phase]||colors.brain,depthTest:false,transparent:true,opacity:.8}));this.connection.renderOrder=2;this.scene.add(this.connection);this.marker=new THREE.Mesh(new THREE.SphereGeometry(.04,12,8),new THREE.MeshBasicMaterial({color:'#f6dfa0',depthTest:false}));this.marker.renderOrder=3;this.scene.add(this.marker);
   this.signalTrain=new THREE.InstancedMesh(new THREE.SphereGeometry(.034,10,8),new THREE.MeshBasicMaterial({color:'#ffe1a4',depthTest:false}),7);this.signalTrain.renderOrder=4;this.signalTrain.frustumCulled=false;this.scene.add(this.signalTrain);
   this.bundle=new THREE.Group();for(let i=0;i<6;i++){const shift=new THREE.Vector3((i-2.5)*.018,Math.sin(i)*.025,(i-2.5)*.016),c=new THREE.CatmullRomCurve3([from.clone().add(shift),mid.clone().add(shift),to.clone().add(shift)]);const fiber=new THREE.Line(new THREE.BufferGeometry().setFromPoints(c.getPoints(45)),new THREE.LineBasicMaterial({color:colors[this.step.phase]||colors.brain,transparent:true,opacity:.33,depthTest:false}));fiber.renderOrder=2;this.bundle.add(fiber)}this.scene.add(this.bundle);
  }
  const addLabel=(p,i,current)=>{if(!p)return;const button=document.createElement('button');button.className='pathway-callout'+(current?' current':'');button.textContent=steps[i].name[document.documentElement.lang==='bn'?'bn':'en'];button.onclick=()=>this.onSelect(i);this.overlay.append(button);this.routeLabels.push({element:button,position:p,index:i,current})};
  if(fromIndex!==null&&to&&from.distanceTo(to)>.35)addLabel(from,fromIndex,false);addLabel(to,index,true);this.resize();
 }
