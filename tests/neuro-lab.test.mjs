...
test('the current step determines the model even when a topic has a common default',()=>{
 const model=(topic,id,concept='example')=>mechanismFor({topic:{id:topic},concept:{id:concept}},{id});
 assert.equal(model('eeg','sound','erp'),'hearing');
 assert.equal(model('chemistry','hand-muscle','acetylcholine'),'muscle');
 assert.equal(model('chemistry','skin','dopamine'),'receptor');
 assert.equal(model('computational','somatic'),null);
 for(const id of ['implant-record','decoder','prosthetic','bci-feedback'])assert.equal(model('bci',id),'bci');
 assert.equal(model('bci','eeg-record','eeg-interface'),'rhythm');
 assert.equal(model('bci','motor'),null);
 assert.equal(model('sensory','optic','vision'),'vision');
});
...
test('every mapped lesson stage has bilingual model guidance',()=>{
 let stages=0,concepts=0;const kinds=new Set();
 for(const topic of topics)for(const concept of topic.items){const journey=getJourney(topic,concept);if(!journey)continue;concepts++;for(const step of journey.steps){const kind=mechanismFor(journey,step);stages++;if(!kind)continue;kinds.add(kind);for(const language of ['en','bn']){assert(labCopy[kind]?.title[language]);assert(labCopy[kind]?.note[language]);assert.equal(labCopy[kind].legend.length,3)}}}
 assert.equal(concepts,193);assert(stages>concepts);assert(kinds.has('bci'));assert(kinds.has('gap'));assert(kinds.has('spindle'));
});
